 ...
 *   multiple values). The hook should
 *   take this parameter by reference and alter it. In the usual case,
 *   if a field managed by the implementing module has values that indicate
 *   that n variants should be created, the module should replace every
 *   member of $combinations with n copies of that member, one for each
 *   variant of the module. The first hook implementation that will be called
 *   will have a value of array(array()) for this parameter, meaning there
 *   is only one (empty) combination.
 */
...
function hook_commerce_bpc_tokens($product_type, $combination, $options) {
  ...
  foreach ($combination as $field_name => $values) {
    ...
    if ($field['module'] == 'list' &&  !_commerce_bpc_get_value($instance, array('commerce_bpc', 'is_static'), FALSE)) {
      ...
      $items = reset($values);
      // We only allow a single value per combination, so we can treat this like
